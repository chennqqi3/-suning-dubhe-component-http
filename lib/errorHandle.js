        ...
        function ($q, errors, AlertService, LoginService) {
            ...
            return {
                handle: function (data) {
                    ...
                    if (option.resultCode && option.resultCode.length) {
                        for (let i = 0; i < option.resultCode.length; i++) {
                            if (data.resultCode === option.resultCode[i].code) {
                                e.callback && e.callback(defer, data)
                            }
                        }
